...
QiniuService.prototype.upload = function (req, res, options, cb) {
  ...
  busboy.on('file', function (fieldname, file, filename, encoding, mimetype) {
    ...
    qiniu.io.putReadable(self.getToken(bucket, key), key, file, null, function (err, ret) {
      if (err) {
        cb(err)
      }
    })
...
QiniuService.prototype.download = function (container, filename, requet, response, cb) {
  const self = this
  let bucket, key
  ...
  if (self.options.domain) {
    ...
    .then(function (res) {
      const domain = self.options.bucket && self.options.domain
        ? self.options.domain
        : res.data[0]
      if (domain) {
        const baseUrl = qiniu.rs.makeBaseUrl(domain, filename)

        const policy = new qiniu.rs.GetPolicy()
        response.redirect(policy.makeRequest(baseUrl))
      } else {
